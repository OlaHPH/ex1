...
<p>
The password manager I chose here is 1Pass. In general, password managers are apps which generate strong passwords, securely store them and automatically fill them in on websites/apps where login is required. 1Pass is built based on the open-source SQLite database format and uses algorithms that expers have examined and verified to keep information safe (published algorithms???) 

* What threats does it protect against? password (and login details) exposure, data breach
* What information is encrypted, what's not? - credentials 8account password,  secret key) are never sent over the network 
